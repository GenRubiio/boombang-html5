...
class UserInteractionAnimation {
    static main(spriteAvatar, direction, senderUser, avatarId, gameScene, interactionType) {
        if (UserEmojiAnimation.isFlying(spriteAvatar)) {
            UserEmojiAnimation.cancelFly(spriteAvatar, gameScene);
        }
        const textureKey = this.#textureKey(senderUser, direction, avatarId, interactionType);
    ...
    static #textureKey(senderUser, direction, avatarId, interactionType) {
        switch (interactionType) {
            ...
            case 'rose':
                switch (avatarId) {
                    case AvatarEnum.GATA:
                        return senderUser ? direction + "down_flor" : direction + "_flor_rec";
                    default:
