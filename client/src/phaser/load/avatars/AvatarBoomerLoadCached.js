...
class AvatarBoomerLoadCached {
    /**
     * Carga el avatar Boomer usando el sistema de cache
     */
    static async main(gameScene, avatarId) {
        try {
            ...
            console.log(`🚀 Avatar Boomer ${avatarId} configurado con sistema de cache`);
        } catch (error) {
            console.warn(`⚠️ Error cargando avatar Boomer con cache, usando método tradicional:`, error);
            this.loadTraditional(gameScene, avatarId);
        }
    }

    static async loadCachedAtlas(gameScene, avatarId) {
        const atlasKey = 'boomer_atlas';
    ...
    static async isAvailableInCache() {
        try {
            ...
            const avatarId = 1; // BOOMER
            const version = assetVersionManager.getAvatarVersion(avatarId);
            const hasAtlas = await cacheManager.hasAsset(`boomer_atlas_v${version}`, cacheManager.stores.ATLAS);
            const hasSpreadsheet = await cacheManager.hasAsset(`boomer_spreadsheet_v${version}`, cacheManager.stores.SPREADSHEET);
...
export default AvatarBoomerLoadCached;