...
class AvatarGataLoadCached {
    /**
     * Carga el avatar Gata usando el sistema de cache
     */
    static async main(gameScene, avatarId) {
        try {
            ...
            console.log(`🚀 Avatar Gata ${avatarId} configurado con sistema de cache`);
        } catch (error) {
            console.warn(`⚠️ Error cargando avatar Gata con cache, usando método tradicional:`, error);
    ...
    static async isAvailableInCache() {
        try {
            ...
            const avatarId = 5; // GATA
            const version = assetVersionManager.getAvatarVersion(avatarId);
            const versionedAtlasKey = `gata_atlas_v${version}`;
            const versionedSpreadsheetKey = `gata_spreadsheet_v${version}`;
    ...
    /**
     * Obtiene estadísticas específicas del avatar Gata en cache
     */
    ...
    /**
     * Limpia el cache específico del avatar Gata
     */
    static async clearCache() {
        try {
            ...
            console.log('🗑️ Cache del avatar Gata limpiado');
            return true;
        } catch (error) {
            console.warn('⚠️ Error limpiando cache del avatar Gata:', error);
            return false;
        }
    }
}

export default AvatarGataLoadCached;