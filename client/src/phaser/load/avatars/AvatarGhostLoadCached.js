...
import asset_spritesheet1_image from '@/assets/game/avatars/ghost/animations/spritesheet-1.webp';
...
class AvatarGhostLoadCached {
    /**
     * Carga el avatar Ghost usando el sistema de cache
     */
    static async main(gameScene, avatarId) {
        try {
            ...
            console.log(`🚀 Avatar Ghost ${avatarId} configurado con sistema de cache`);
        } catch (error) {
            console.warn(`⚠️ Error cargando avatar Ghost con cache, usando método tradicional:`, error);
            this.loadTraditional(gameScene, avatarId);
        }
    }

    static async loadCachedAtlas(gameScene, avatarId) {
        const atlasKey = 'ghost_atlas';
    ...
    static async isAvailableInCache() {
        try {
            ...
            const avatarId = 6; // GHOST
            const version = assetVersionManager.getAvatarVersion(avatarId);
            const hasAtlas = await cacheManager.hasAsset(`ghost_atlas_v${version}`, cacheManager.stores.ATLAS);
            const hasSpreadsheet = await cacheManager.hasAsset(`ghost_spreadsheet_v${version}`, cacheManager.stores.SPREADSHEET);
...
export default AvatarGhostLoadCached;