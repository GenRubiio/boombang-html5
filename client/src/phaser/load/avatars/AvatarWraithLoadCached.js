...
import asset_spritesheet1_image from '@/assets/game/avatars/wraith/animations/spritesheet-1.webp';
...
class AvatarWraithLoadCached {
    /**
     * Carga el avatar Wraith usando el sistema de cache
     */
    static async main(gameScene, avatarId) {
        try {
            ...
            console.log(`🚀 Avatar Wraith ${avatarId} configurado con sistema de cache`);
        } catch (error) {
            console.warn(`⚠️ Error cargando avatar Wraith con cache, usando método tradicional:`, error);
            this.loadTraditional(gameScene, avatarId);
        }
    }

    static async loadCachedAtlas(gameScene, avatarId) {
        const atlasKey = 'wraith_atlas';
    ...
    static async isAvailableInCache() {
        try {
            ...
            const avatarId = 10; // WRAITH
            const version = assetVersionManager.getAvatarVersion(avatarId);
            const hasAtlas = await cacheManager.hasAsset(`wraith_atlas_v${version}`, cacheManager.stores.ATLAS);
            const hasSpreadsheet = await cacheManager.hasAsset(`wraith_spreadsheet_v${version}`, cacheManager.stores.SPREADSHEET);
...
export default AvatarWraithLoadCached;