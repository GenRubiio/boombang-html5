import asset_spritesheet0_image from '@/assets/game/avatars/yayo/animations/spritesheet-0.webp';
import asset_spritesheet1_image from '@/assets/game/avatars/yayo/animations/spritesheet-1.webp';
import asset_atlas_json from '@/assets/game/avatars/yayo/animations/atlas.json';
import asset_cara_peque_image from '@/assets/game/avatars/yayo/cara_peque.svg';
import asset_cara_media_image from '@/assets/game/avatars/yayo/cara_media.svg';
import CachedAtlasLoader from '../CachedAtlasLoader';

class AvatarYayoLoadCached {
    /**
     * Carga el avatar Yayo usando el sistema de cache
     */
    static async main(gameScene, avatarId) {
        try {
            ...
            console.log(`🚀 Avatar Yayo ${avatarId} configurado con sistema de cache`);
        } catch (error) {
            console.warn(`⚠️ Error cargando avatar Yayo con cache, usando método tradicional:`, error);
            this.loadTraditional(gameScene, avatarId);
        }
    }

    static async loadCachedAtlas(gameScene, avatarId) {
        const atlasKey = 'yayo_atlas';
    ...
    static loadTraditional(gameScene, avatarId) {
        ...
        gameScene.load.multiatlas('yayo_atlas', asset_atlas_json);
        gameScene.load.svg((avatarId + "_cara_peque"), asset_cara_peque_image);
        gameScene.load.svg((avatarId + "_cara_media"), asset_cara_media_image);
    }
    ...
    static async isAvailableInCache() {
        try {
            ...
            const avatarId = 12; // YAYO
            const version = assetVersionManager.getAvatarVersion(avatarId);
            const hasAtlas = await cacheManager.hasAsset(`yayo_atlas_v${version}`, cacheManager.stores.ATLAS);
            const hasSpreadsheet = await cacheManager.hasAsset(`yayo_spreadsheet_v${version}`, cacheManager.stores.SPREADSHEET);
...
export default AvatarYayoLoadCached;