...
@section('og-image'){{ !is_array($object) && isset($object->og_image) ? $object->og_image : asset(config('settings.logo')) }}@endsection
...
@section('tw-image'){{ !is_array($object) && isset($object->tw_image) ? $object->tw_image : asset(config('settings.logo')) }}@endsection
