@php
    ...
    if (!is_null(old(square_brackets_to_dots($field['name'])))) {
        ...
        $value = old_empty_or_null($field['name'], '') ??  url($field['value'] ?? '') ?? $field['default'] ?? '';
    }
@endphp
...
@push('crud_fields_styles')
    ...
    @bassetBlock('bpFieldInitBase64CropperImageCss')
    <style>
        .hide {
            display: none;
        }
...
@push('crud_fields_scripts')
    ...
    @bassetBlock('bpFieldInitBase64CropperImageElement')
    <script>
