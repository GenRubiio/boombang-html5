<div class="mb-2 text-right d-none" id="modal-change-lang_section_modal" data-lang="{{ app()->getLocale() }}" data-default-lang="{{ app()->getLocale() }}">
    <div class="btn-group">
        <button type="button" class="btn btn-sm btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true"
            aria-expanded="false">
            {{ trans('backpack::crud.language') }}:
            <span class="current-language_section_modal">
        ...
        <ul class="dropdown-menu">
            @foreach (config('backpack.crud.locales') as $key => $locale)
                <button class="dropdown-item language_section_modal-option_button" data-local="{{ $key }}"
